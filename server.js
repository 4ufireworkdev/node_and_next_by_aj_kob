...
app.get("/user/verifyToken", (req, res) => {
  const secret = process.env.TOKEN_SECRET;
  const token =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MTAwLCJuYW1lIjoiRmlyZXdvcmsiLCJsZXZlbCI6ImFkbWluIiwiaWF0IjoxNzI2NjU4OTAxLCJleHAiOjE3MjY3NDUzMDF9.FC8Llb2vx4trO8wzNAMC8eDqwz3xXjZzB7OG1cEy_yI";
...
app.get("/writeFile", (req, res) => {
  try {
    const fs = require("fs");
    fs.writeFile("test.txt", "hello by Firework", (err, data) => {
      if (err) {
        throw err;
      }
      res.send({ message: "success" });
...
app.get("/createPdf", (req, res) => {
  try {
    ...
    doc.addPage().fontSize(25).text("ภาษาไทย หน้า 2", 100, 100);
    doc.end();
    res.send({ message: "success" });
    res.send({ found: found });
  } catch (e) {
